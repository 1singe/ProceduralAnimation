...
void RenderApi3D::buffer(const Buffer3D& buffer, eDrawMode drawMode, glm::mat4 const* pModel) const {
	...
	glProgramUniformMatrix4fv(pRenderEngine->shader3D.programId, pRenderEngine->shader3D.modelLocation, 1, 0, glm::value_ptr(model));

	const bool lightingEnabled = buffer.vbos[Buffer3D::BufferAttribNormal] != 0;
	glProgramUniform1i(pRenderEngine->shader3D.programId, pRenderEngine->shader3D.lightingEnabledLocation, lightingEnabled);
